...
class iso _TestList is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let b = a.clone()
    h.assert_eq(b.size(), 3)
    h.assert_eq(b(0), 0)
    h.assert_eq(b(1), 1)
    h.assert_eq(b(2), 2)

    b.remove(1)
    h.assert_eq(b.size(), 2)
    h.assert_eq(b(0), 0)
    h.assert_eq(b(1), 2)

    b.append_list(a)
    h.assert_eq(b.size(), 5)
    h.assert_eq(b(0), 0)
    h.assert_eq(b(1), 2)
    h.assert_eq(b(2), 0)
    h.assert_eq(b(3), 1)
...
class iso _TestListsFrom is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let b = List.from(Array[U32])
    h.assert_eq(b.size(), 0)
...
class iso _TestMap is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq("1", a_removed_pair._1)
...
class iso _TestMapRemove is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    x(1) = "one"
    h.assert_eq(x.size(), 1)
    x.remove(1)
    h.assert_eq(x.size(), 0)
...
class iso _TestMapUpsert is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq("1", x2(1))
    x2.upsert(1, "2", g)
    h.assert_eq("1, 2", x2(1))
    x2.upsert(1, "3", g)
    h.assert_eq("1, 2, 3", x2(1))
    x2.upsert(1, "abc", g)
    h.assert_eq("1, 2, 3, abc", x2(1))
    x2.upsert(1, "def", g)
    h.assert_eq("1, 2, 3, abc, def", x2(1))
    ...
    let f' = {(x: U64, y: U64): U64 => x + y }
    h.assert_eq(expected_initial_size, x3.space())
    ...
    h.assert_ne(expected_initial_size, x3.space())
...
class iso _TestListsFold is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq(resList.size(), 4)
    h.assert_eq(resList(0), 0)
    h.assert_eq(resList(1), 2)
    h.assert_eq(resList(2), 4)
    h.assert_eq(resList(3), 6)
...
class iso _TestListsEvery is UnitTest
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_eq(lessThan4, true)
    h.assert_eq(lessThan3, false)
    h.assert_eq(lessThan0, false)

    let b = List[U32]
    let empty = b.every(f)
    h.assert_eq(empty, true)
...
class iso _TestListsExists is UnitTest
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_eq(gt2, true)
    h.assert_eq(gte0, true)
    h.assert_eq(lt0, false)

    let b = List[U32]
    let empty = b.exists(f)
    h.assert_eq(empty, false)
...
class iso _TestListsPartition is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq(evens.size(), 2)
    h.assert_eq(evens(0), 0)
    h.assert_eq(evens(1), 2)
    h.assert_eq(odds.size(), 2)
    h.assert_eq(odds(0), 1)
    h.assert_eq(odds(1), 3)

    let b = List[U32]
    (let emptyEvens, let emptyOdds) = b.partition(isEven)

    h.assert_eq(emptyEvens.size(), 0)
    h.assert_eq(emptyOdds.size(), 0)
...
class iso _TestListsDrop is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq(b(0), 2)
    h.assert_eq(b(2), 4)
    h.assert_eq(c.size(), 1)
    h.assert_eq(c(0), 4)
    h.assert_eq(d.size(), 0)
    h.assert_eq(e.size(), 0)

    let empty = List[U32]
    let l = empty.drop(3)
    h.assert_eq(l.size(), 0)
...
class iso _TestListsTake is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_eq(b(0), 0)
    h.assert_eq(b(1), 1)
    h.assert_eq(c.size(), 4)
    h.assert_eq(c(0), 0)
    h.assert_eq(c(1), 1)
    h.assert_eq(c(2), 2)
    h.assert_eq(c(3), 3)
    h.assert_eq(d.size(), 5)
    h.assert_eq(d(0), 0)
    h.assert_eq(d(1), 1)
    h.assert_eq(d(2), 2)
    h.assert_eq(d(3), 3)
    h.assert_eq(d(4), 4)
    h.assert_eq(e.size(), 5)
    h.assert_eq(e(0), 0)
    h.assert_eq(e(1), 1)
    h.assert_eq(e(2), 2)
    h.assert_eq(e(3), 3)
    h.assert_eq(e(4), 4)
    h.assert_eq(m.size(), 0)
...
class iso _TestListsContains is UnitTest
  ...
  fun apply(h: TestHelper) =>
    ...
    h.assert_eq(a.contains(0), true)
    h.assert_eq(a.contains(1), true)
    h.assert_eq(a.contains(2), true)
    h.assert_eq(a.contains(3), false)
